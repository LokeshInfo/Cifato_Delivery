...
import com.ics.cifato_delivery.View.Main_Activity;
import com.ics.cifato_delivery.View.Splash_Activity;
...
//        Log.i("info", "Notification Message Body: " + remoteMessage.getNotification().getBody());
      //  sendNotification(remoteMessage.get,remoteMessage.getFrom(),remoteMessage.getMessageId(),remoteMessage.getData().toString());
        ...
        if (remoteMessage.getData().size() >0) {
            ...
            try {
            String messg = (object.getString("message"));
            messg = messg.substring(messg.lastIndexOf(" ")+1);
            String block_messg = "BLOCK";
            Log.e("BLOCK MESSAGE > "," ."+messg+". ----------- ."+block_messg+".");
            if (messg.equals(block_messg))
            {
